...
class LoginUI:
    ...
    def __init__(self, master):
        ...
        self.frame = ctk.CTkFrame(
            master, width=400, height=400, corner_radius=16)
        self.frame.pack(expand=True)
        self.frame.pack_propagate(False)  # Impede o frame de encolher

        # Título
        self.label_title = ctk.CTkLabel(
            self.frame, text="Login CQ_Lab", font=("Arial", 22))
        self.label_title.pack(pady=(30, 20))

        # Campo Usuário
        self.entry_user = ctk.CTkEntry(
            self.frame, placeholder_text="Usuário", width=260)
        ...
        self.entry_pass = ctk.CTkEntry(
            self.frame, placeholder_text="Senha", show="*", width=260)
        self.entry_pass.pack(pady=10)

        # Botão Entrar
        self.button_login = ctk.CTkButton(
            self.frame, text="Entrar", command=self.login, width=260)
        self.button_login.pack(pady=15)

        # Botão Novo Usuário
        self.button_cadastro = ctk.CTkButton(
            self.frame, text="Novo Usuário", command=self.abrir_cadastro, width=260)
        self.button_cadastro.pack(pady=10)

        # Botão Esqueci a Senha
        self.button_reset = ctk.CTkButton(
            self.frame, text="Esqueci a senha", command=self.abrir_reset, width=260)
    ...
    def login(self):
        ...
        if autenticar_usuario(usuario, senha):
            ...
            self.label_status.configure(
                text="Usuário ou senha inválidos", text_color="red")
    ...
    def abrir_cadastro(self):
        ...
        # Fica à frente da tela de login
        janela_cadastro.transient(self.master)
        ...
        frame_cadastro = ctk.CTkFrame(
            janela_cadastro, width=420, height=440, corner_radius=16)
        frame_cadastro.pack(expand=True)
        frame_cadastro.pack_propagate(False)


        label_titulo = ctk.CTkLabel(
            frame_cadastro, text="Criar Novo Usuário", font=("Arial", 18), width=240)
        label_titulo.pack(pady=(30, 20))

        entry_novo_user = ctk.CTkEntry(
            frame_cadastro, placeholder_text="Usuário", width=240)
        entry_novo_user.pack(pady=10)

        entry_nova_senha = ctk.CTkEntry(
            frame_cadastro, placeholder_text="Senha", show="*", width=240)
        entry_nova_senha.pack(pady=10)

        entry_confirmar = ctk.CTkEntry(
            frame_cadastro, placeholder_text="Confirmar Senha", show="*", width=240)
        ...
        lbl_len = ctk.CTkLabel(
            checklist_frame, text="  • Mínimo 8 caracteres  ", text_color="white",
            width=240,
            corner_radius=8)
        lbl_len.pack(anchor="w")
        lbl_alpha = ctk.CTkLabel(
            checklist_frame, text="  • Contém letra  ", text_color="white",
            width=240,
            corner_radius=8)
        lbl_alpha.pack(anchor="w")
        lbl_digit = ctk.CTkLabel(
            checklist_frame, text="  • Contém número  ", text_color="white",
            width=240,
            corner_radius=8)
        ...
        def _update_checklist(event=None):
            ...
            lbl_alpha.configure(text_color=(
                "green" if any(c.isalpha() for c in s) else "red"))
            lbl_digit.configure(text_color=(
                "green" if any(c.isdigit() for c in s) else "red"))
        ...
        def cadastrar():
            ...
            if senha != confirmar:
                label_msg.configure(
                    text="As senhas não coincidem", text_color="red")
                return
            ...
            if created:
                label_msg.configure(
                    text="Usuário criado com sucesso!", text_color="green")
                entry_novo_user.delete(0, "end")
                entry_nova_senha.delete(0, "end")
                entry_confirmar.delete(0, "end")
            else:
                label_msg.configure(
                    text="Usuário já existe!", text_color="red")

        button_criar = ctk.CTkButton(
            frame_cadastro, text="Cadastrar", command=cadastrar, width=240)
    ...
    def abrir_reset(self):
        ...
        frame = ctk.CTkFrame(janela, width=420, height=440, corner_radius=16)
        frame.pack(expand=True)
        frame.pack_propagate(False)

        lbl = ctk.CTkLabel(frame, text="Resetar Senha", font=("Arial", 16), width=240)
        lbl.pack(pady=(20, 10))

        entry_user = ctk.CTkEntry(frame, placeholder_text="Usuário", width=240)
        entry_user.pack(pady=10)

        entry_nova = ctk.CTkEntry(
            frame, placeholder_text="Nova senha", show="*", width=240)
        entry_nova.pack(pady=10)

        entry_conf = ctk.CTkEntry(
            frame, placeholder_text="Confirmar nova senha", show="*", width=240)
        ...
        def confirmar_reset():
            ...
            if not validar_senha(s):
                lbl_msg.configure(
                    text="Senha deve ter >=8 caracteres e conter letras e números", text_color="red")
                return
            # tentar resetar
            try:
                ...
                if ok:
                    lbl_msg.configure(
                        text="Senha atualizada com sucesso", text_color="green")
                    entry_user.delete(0, 'end')
                    entry_nova.delete(0, 'end')
                    entry_conf.delete(0, 'end')
                else:
                    lbl_msg.configure(
                        text="Usuário não encontrado", text_color="red")
            except ValueError as ve:
                lbl_msg.configure(text=str(ve), text_color="red")

        btn = ctk.CTkButton(frame, text="Confirmar",
                            command=confirmar_reset, width=240)
        btn.pack(pady=5)
