...
class MainMenu:
    def __init__(self, master, usuario):
        ...
        self.center_frame = ctk.CTkFrame(self.frame, width=380, height=340, corner_radius=16)
        self.center_frame.place(relx=0.5, rely=0.5, anchor="center")
        self.center_frame.pack_propagate(False)

        self.label = ctk.CTkLabel(self.center_frame, text=f"Bem-vindo, {usuario}!", font=("Arial", 22))
        self.label.pack(pady=(30, 20))

        # Botões principais
        self.btn_solos = ctk.CTkButton(self.center_frame, text="Solos", command=self.abrir_solos, width=220)
        self.btn_solos.pack(pady=10, padx=30)

        self.btn_concreto = ctk.CTkButton(self.center_frame, text="Concreto", command=self.abrir_concreto, width=220)
        self.btn_concreto.pack(pady=10, padx=30)

        self.btn_asfalto = ctk.CTkButton(self.center_frame, text="Asfalto", command=self.abrir_asfalto, width=220)
        self.btn_asfalto.pack(pady=10, padx=30)

        self.btn_sair = ctk.CTkButton(self.center_frame, text="Sair", command=self.sair, width=180)
        self.btn_sair.pack(pady=20, padx=30)
